 ...
 */
abstract class MUSound_Block_Base_AbstractItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    public function display($blockinfo)
    {
        ...
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            $catProperties = ModUtil::apiFunc('MUSound', 'category', 'getAllProperties', array('ot' => $objectType));
            // apply category filters
            if (is_array($properties['catIds']) && count($properties['catIds']) > 0) {
                $qb = ModUtil::apiFunc('MUSound', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $properties['catIds']));
            }
        }
    ...
    protected function getSortParam(array $properties, $repository)
    {
        ...
        if ($properties['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('MUSound', 'selection', 'getIdFields', array('ot' => $properties['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $primaryRegistry = ModUtil::apiFunc('MUSound', 'category', 'getPrimaryProperty', array('ot' => $properties['objectType']));
        $properties['catIds'] = array($primaryRegistry => array());
        if (in_array($properties['objectType'], $this->categorisableObjectTypes)) {
            $properties['catIds'] = ModUtil::apiFunc('MUSound', 'category', 'retrieveCategoriesFromRequest', array('ot' => $properties['objectType']));
        }
