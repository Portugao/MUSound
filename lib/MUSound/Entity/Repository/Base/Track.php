 ...
 *
 * This is the base repository class for track entities.
 */
class MUSound_Entity_Repository_Base_Track extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('track', 'workflowState');
            }
    
            // initialise Imagine preset instances
            $imageHelper = new MUSound_Util_Image($serviceManager);
    
            $objectType = 'track';
            $templateParameters[$objectType . 'ThumbPresetUploadTrack'] = $imageHelper->getPreset($objectType, 'uploadTrack', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetUploadZip'] = $imageHelper->getPreset($objectType, 'uploadZip', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUSound_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['album'] = isset($this->controllerArguments['album']) ? $this->controllerArguments['album'] : FormUtil::getPassedValue('album', 0, 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUSound_Entity_Track', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUSound_Entity_Track', 'tbl')
     ...
     *
     * @return array|trackEntity retrieved data array or trackEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved trackEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function getSelectWherePaginatedQuery(QueryBuilder $qb, $currentPage = 1, $resultsPerPage = 25)
    {
        ...
        $paginatorClass = 'MUSound_Paginator_' . (!$isLegacy ? 'Paginator' : 'LegacyPaginator');
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('MUSoundTracksCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('MUSoundTracksCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUSound', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'track', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved tracks only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.author LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.uploadTrack = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.uploadZip = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    public function retrieveCollectionResult(Query $query, $orderBy = '', $isPaginated = false)
    {
        ...
        if ($isLegacy) {
            ...
            if (!$isPaginated) {
                ...
                $paginatorClass = 'MUSound_Paginator_Paginator';
                $hasRelationships = true;
                $paginator = new $paginatorClass($query, $hasRelationships);
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numTracks';
        if ($useJoins === true) {
     ...
     * @param int    $excludeId  Id of tracks to exclude (optional)
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUSound', 'showOnlyOwnEntries', 0), 'GETPOST');
