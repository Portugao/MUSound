 ...
 */
abstract class MUSound_Entity_Validator_Base_AbstractAlbum extends MUSound_Validator
{
    /**
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('uploadCover', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('upload cover', 255), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotLongerThan('publishedText', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('published text', 255), $dom);
            return $errorInfo;
        }
