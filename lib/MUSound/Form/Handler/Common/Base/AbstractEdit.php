 ...
 */
abstract class MUSound_Form_Handler_Common_Base_AbstractEdit extends Zikula_Form_AbstractHandler
{
    /**
    ...
    public function initialize(Zikula_Form_View $view)
    {
        ...
        $controllerHelper = new MUSound_Util_Controller($this->view->getServiceManager());
    ...
    protected function initTranslationsForEditing()
    {
        ...
        $translatableHelper = new MUSound_Util_Translatable($this->view->getServiceManager());
    ...
    public function handleCommand(Zikula_Form_View $view, &$args)
    {
        ...
        if ($entity->supportsHookSubscribers() && $action != 'cancel') {
            $hookHelper = new MUSound_Util_Hook($this->view->getServiceManager());
    ...
    protected function writeRelationDataToEntity_rec($entity, $entityData, $plugins)
    {
        foreach ($plugins as $plugin) {
            if ($plugin instanceof MUSound_Form_Plugin_AbstractObjectSelector && method_exists($plugin, 'assignRelatedItemsToEntity')) {
    ...
    {
        foreach ($plugins as $plugin) {
            if ($plugin instanceof MUSound_Form_Plugin_AbstractObjectSelector && method_exists($plugin, 'persistRelatedItems')) {
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $roles['isModerator'] = ModUtil::apiFunc('Groups', 'user', 'isgroupmember', $groupArgs);
    
        if (true === $enterprise) {
            $groupArgs = array('uid' => $uid, 'gid' => $this->getVar('superModerationGroupFor' . $this->objectTypeCapital, 2));
            $roles['isSuperModerator'] = ModUtil::apiFunc('Groups', 'user', 'isgroupmember', $groupArgs);
        }
    ...
    protected function handleUploads($formData, $entity)
    {
        ...
        $uploadHandler = new MUSound_UploadHandler();
