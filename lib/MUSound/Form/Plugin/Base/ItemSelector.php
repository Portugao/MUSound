 ...
 */
class MUSound_Form_Plugin_Base_ItemSelector extends Zikula_Form_Plugin_TextInput
{
    /**
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('MUSound'));
        }
        $firstTime = false;

        if (!SecurityUtil::checkPermission('MUSound:' . ucwords($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }

        $categorisableObjectTypes = array('album');
        $catIds = array();
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            ...
            $catIds = ModUtil::apiFunc('MUSound', 'category', 'retrieveCategoriesFromRequest', array('ot' => $this->objectType));
        }
        ...
        $properties = null;
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('MUSound', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    ...
    {
        parent::decode($view);
        $this->objectType = FormUtil::getPassedValue('MUSound_objecttype', 'collection', 'POST');
        $this->selectedItemId = $this->text;
    }
}
