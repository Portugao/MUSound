 ...
 */
class MUSound_ContentType_Base_ItemList extends Content_AbstractContentType
{
    /**
    ...
    public function getTitle()
    {
        ...
        return __('MUSound list view', $dom);
    }
    ...
    public function getDescription()
    {
        ...
        return __('Display list of MUSound objects.', $dom);
    }
    ...
    public function loadData(&$data)
    {
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            $idFields = ModUtil::apiFunc('MUSound', 'selection', 'getIdFields', array('ot' => $this->objectType));
            $this->catRegistries = ModUtil::apiFunc('MUSound', 'category', 'getAllPropertiesWithMainCat', array('ot' => $this->objectType, 'arraykey' => $idFields[0]));
            $this->catProperties = ModUtil::apiFunc('MUSound', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    
        if (!isset($data['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('MUSound', 'category', 'getPrimaryProperty', array('ot' => $this->objectType));
    ...
    public function display()
    {
        ...
        $entityClass = 'MUSound_Entity_' . ucwords($this->objectType);
        $serviceManager = ServiceUtil::getManager();
        $entityManager = $serviceManager->getService('doctrine.entitymanager');
        $repository = $entityManager->getRepository($entityClass);
    
        // ensure that the view does not look for templates in the Content module (#218)
        $this->view->toplevelmodule = 'MUSound';
    
        $this->view->setCaching(Zikula_View::CACHE_ENABLED);
        // set cache id
        $component = 'MUSound:' . ucwords($this->objectType) . ':';
        $instance = '::';
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            if (is_array($this->catIds) && count($this->catIds) > 0) {
                $qb = ModUtil::apiFunc('MUSound', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $this->objectType, 'catids' => $this->catIds));
            }
        }
    ...
    public function startEditing()
    {
        ...
        array_push($this->view->plugins_dir, 'modules/MUSound/templates/plugins');
