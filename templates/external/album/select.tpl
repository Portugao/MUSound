...
{if $properties ne null && is_array($properties)}
    ...
    {foreach key='propertyName' item='propertyId' from=$properties}
        <p>
            {modapifunc modname='MUSound' type='category' func='hasMultipleSelection' ot='album' registry=$propertyName assign='hasMultiSelection'}
            ...
            &nbsp;
            {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='MUSound' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
            <br{$break} />
        </p>
    {/foreach}
    {/nocache}
{/if}
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Album'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='album' from=$items}
            <option value="{$album.id}"{if $selectedId eq $album.id} selected="selected"{/if}>{$album->getTitleFromDisplayPattern()}</option>
        {foreachelse}
