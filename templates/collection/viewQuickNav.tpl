...
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mUSoundCollectionQuickNavForm" class="musound-quicknav">
    <fieldset>
        ...
        {/if}
        <input type="submit" name="updateview" id="musound_collectionquicknav_updateview" value="{gt text='OK'}" />
    </fieldset>
</form>
<script type="text/javascript">
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mUMUSoundInitQuickNavigation('collection');
        {{if isset($searchFilter) && $searchFilter eq false}}
            {{* we can hide the submit button if we have no quick search field *}}
            $('musound_collectionquicknav_updateview').addClassName('z-hide');
        {{/if}}
    });
/* ]]> */
</script>
