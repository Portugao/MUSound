{* purpose of this template: tracks view view in admin area *}
...
<div class="musound-track musound-view">
    ...
    {if $canBeCreated}
        {checkpermissionblock component='MUSound:Track:' instance='::' level='ACCESS_EDIT'}
            ...
            <a href="{modurl modname='MUSound' type='admin' func='edit' ot='track'}" title="{$createTitle}" class="z-icon-es-add">{$createTitle}</a>
    ...
    {if isset($showAllEntries) && $showAllEntries eq 1}
        ...
        <a href="{modurl modname='MUSound' type='admin' func='view' ot='track'}" title="{$linkTitle}" class="z-icon-es-view">
            {$linkTitle}
        </a>
        {assign var='all' value=1}
    {else}
        {gt text='Show all entries' assign='linkTitle'}
        <a href="{modurl modname='MUSound' type='admin' func='view' ot='track' all=1}" title="{$linkTitle}" class="z-icon-es-view">{$linkTitle}</a>
    {/if}

    {include file='admin/track/view_quickNav.tpl' all=$all own=$own workflowStateFilter=false}{* see template file for available options *}

    <form action="{modurl modname='MUSound' type='admin' func='handleSelectedEntries'}" method="post" id="tracksViewForm" class="z-form">
        <div>
            ...
            <table class="z-datatable">
                ...
                <tr>
                    ...
                    <th id="hTitle" scope="col" class="z-left">
                        {sortlink __linktext='Title' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='title' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
                    <th id="hDescription" scope="col" class="z-left">
                        {sortlink __linktext='Description' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='description' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
                    <th id="hAuthor" scope="col" class="z-left">
                        {sortlink __linktext='Author' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='author' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
                    <th id="hUploadTrack" scope="col" class="z-left">
                        {sortlink __linktext='Upload track' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='uploadTrack' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
                    <th id="hUploadZip" scope="col" class="z-left">
                        {sortlink __linktext='Upload zip' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='uploadZip' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
                    <th id="hAlbum" scope="col" class="z-left">
                        {sortlink __linktext='Album' currentsort=$sort modname='MUSound' type='admin' func='view' ot='track' sort='album' sortdir=$sdir all=$all own=$own album=$album workflowState=$workflowState searchterm=$searchterm pageSize=$pageSize}
                    </th>
            ...
            {foreach item='track' from=$items}
                <tr class="{cycle values='z-odd, z-even'}">
                    ...
                    <td headers="hTitle" class="z-left">
                        <a href="{modurl modname='MUSound' type='admin' func='display' ot='track' id=$track.id}" title="{gt text='View detail page'}">{$track.title|notifyfilters:'musound.filterhook.tracks'}</a>
                    </td>
                    ...
                    <td headers="hAlbum" class="z-left">
                        {if isset($track.Album) && $track.Album ne null}
                            ...
                            {/strip}</a>
                            <a id="albumItem{$track.id}_rel_{$track.Album.id}Display" href="{modurl modname='MUSound' type='admin' func='display' ot='album' id=$track.Album.id theme='Printer'}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
            ...
            {if !isset($showAllEntries) || $showAllEntries ne 1}
                {pager rowcount=$pager.numitems limit=$pager.itemsperpage display='page' modname='MUSound' type='admin' func='view' ot='track'}
            {/if}
            <fieldset>
