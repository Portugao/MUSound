{* purpose of this template: tracks atom feed in admin area *}
...
{if $numItems}
{capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$items[0].createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUSound' type='admin' func='display' ot='track' id=$items[0].id}{/capture}
    <id>{$uniqueID}</id>
    <updated>{$items[0].updatedDate|default:$smarty.now|dateformat:'%Y-%m-%dT%H:%M:%SZ'}</updated>
{/if}
    <link rel="alternate" type="text/html" hreflang="{lang}" href="{modurl modname='MUSound' type='admin' func='main' fqurl=1}" />
...
{foreach item='track' from=$items}
    <entry>
        ...
        <link rel="alternate" type="text/html" href="{modurl modname='MUSound' type='admin' func='display' ot='track' id=$track.id fqurl='1'}" />

        {capture assign='uniqueID'}tag:{$baseurl|replace:'http://':''|replace:'/':''},{$track.createdDate|dateformat|default:$smarty.now|dateformat:'%Y-%m-%d'}:{modurl modname='MUSound' type='admin' func='display' ot='track' id=$track.id}{/capture}
        <id>{$uniqueID}</id>
