...
<div class="musound-collection musound-display with-rightbox">
    ...
    <div class="z-admin-content-pagetitle">
        ...
        <h3>{$templateTitle|notifyfilters:'musound.filter_hooks.collections.filter'}{icon id='itemActionsTrigger' type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}</h3>
    </div>
    ...
    {if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
        <div class="musound-rightbox">
            ...
            {if isset($collection.album) && $collection.album ne null}
                {include file='admin/album/include_displayItemListMany.tpl' items=$collection.album}
            {/if}
            
            {checkpermission component='MUSound:Collection:' instance="`$collection.id`::" level='ACCESS_ADMIN' assign='mayManage'}
            ...
            <p class="managelink">
                ...
                <a href="{modurl modname='MUSound' type='admin' func='edit' ot='album' collection="`$collection.id`" returnTo='adminDisplayCollection'}" title="{$createTitle}" class="z-icon-es-add">{$createTitle}</a>
            </p>
            {/if}
        </div>
    {/if}
