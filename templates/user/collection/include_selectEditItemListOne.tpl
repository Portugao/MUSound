{* purpose of this template: inclusion template for display of related collection in user area *}
...
{if isset($item) && is_array($item) && isset($item[0]) && !is_object($item[0])}
    {modapifunc modname='MUSound' type='selection' func='getEntity' objectType='collection' id=$item[0] assign='item'}
{/if}
...
<li id="{$idPrefixItem}">
    ...
    <a id="{$idPrefixItem}Edit" href="{modurl modname='MUSound' type='user' func='edit' ot='collection' id=$item.id forcelongurl=true}">{$editImage}</a>
