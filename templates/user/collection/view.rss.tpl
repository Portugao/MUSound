{* purpose of this template: collections rss feed in user area *}
...
{foreach item='collection' from=$items}
    <item>
        <title><![CDATA[{if isset($collection.updatedDate) && $collection.updatedDate ne null}{$collection.updatedDate|dateformat} - {/if}{$collection->getTitleFromDisplayPattern()|notifyfilters:'musound.filterhook.collections'}]]></title>
        <link>{modurl modname='MUSound' type='user' func='display' ot='collection' id=$collection.id fqurl='1'}</link>
        <guid>{modurl modname='MUSound' type='user' func='display' ot='collection' id=$collection.id fqurl='1'}</guid>
