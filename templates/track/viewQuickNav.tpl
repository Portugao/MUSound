...
<form action="{$modvars.ZConfig.entrypoint|default:'index.php'}" method="get" id="mUSoundTrackQuickNavForm" class="musound-quicknav">
    <fieldset>
        ...
        {if !isset($albumFilter) || $albumFilter eq true}
            ...
            {/php}
            {modapifunc modname='MUSound' type='selection' func='getEntities' ot='album' assign='listEntries'}
        ...
        {/if}
        <input type="submit" name="updateview" id="musound_trackquicknav_updateview" value="{gt text='OK'}" />
    </fieldset>
</form>
<script type="text/javascript">
/* <![CDATA[ */
    document.observe('dom:loaded', function() {
        mUMUSoundInitQuickNavigation('track');
        {{if isset($searchFilter) && $searchFilter eq false}}
            {{* we can hide the submit button if we have no quick search field *}}
            $('musound_trackquicknav_updateview').addClassName('z-hide');
        {{/if}}
    });
/* ]]> */
</script>
