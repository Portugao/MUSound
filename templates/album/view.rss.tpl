...
{*<rss version="0.92">*}
    <channel>
        <title>{gt text='Latest albums'}</title>
        ...
        <description>{gt text='A direct feed showing the list of albums'} - {$modvars.ZConfig.slogan}</description>
...
{foreach item='album' from=$items}
    <item>
        <title><![CDATA[{if isset($album.updatedDate) && $album.updatedDate ne null}{$album.updatedDate|dateformat} - {/if}{$album->getTitleFromDisplayPattern()|notifyfilters:'musound.filterhook.albums'}]]></title>
        <link>{modurl modname='MUSound' type=$lct func='display' ot='album'  id=$album.id fqurl=true}</link>
        <guid>{modurl modname='MUSound' type=$lct func='display' ot='album'  id=$album.id fqurl=true}</guid>
