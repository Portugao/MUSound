...
namespace MU\SoundModule\ContentType\Base;
...
abstract class AbstractItemList extends \Content_AbstractContentType implements ContainerAwareInterface
{
    ...
    {
        return $this->container->get('translator.default')->__('MUSoundModule list view');
    }
    ...
    {
        return $this->container->get('translator.default')->__('Display list of MUSoundModule objects.');
    }
    ...
    {
        $controllerHelper = $this->container->get('mu_sound_module.controller_helper');
        ...
        $featureActivationHelper = $this->container->get('mu_sound_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $categoryHelper = $this->container->get('mu_sound_module.category_helper');
    
            // fetch category properties
            $this->catRegistries = [];
            $this->catProperties = [];
            if (in_array($this->objectType, $this->categorisableObjectTypes)) {
                $entityFactory = $this->container->get('mu_sound_module.entity_factory');
    ...
    {
        $repository = $this->container->get('mu_sound_module.entity_factory')->getRepository($this->objectType);
        $permissionApi = $this->container->get('zikula_permissions_module.api.permission');
    
        // create query
        $orderBy = $this->container->get('mu_sound_module.model_helper')->resolveSortParameter($this->objectType, $this->sorting);
        ...
        $templateParameters = $this->container->get('mu_sound_module.controller_helper')->addTemplateParameters($this->objectType, $templateParameters, 'contentType', []);
    ...
    protected function getDisplayTemplate()
    {
        ...
        foreach ($templateOptions as $templatePath) {
            if ($templating->exists('@MUSoundModule/' . $templatePath)) {
                $template = '@MUSoundModule/' . $templatePath;
                break;
            }
        }
    ...
    public function startEditing()
    {
        ...
        $this->view->toplevelmodule = 'MUSoundModule';
    
        // ensure our custom plugins are loaded
        array_push($this->view->plugins_dir, 'modules/MU/SoundModule/Resources/views/plugins');
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $this->view->assign('categories', $categories)
                       ->assign('categoryHelper', $this->container->get('mu_sound_module.category_helper'));
        }
