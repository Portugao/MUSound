...
abstract class AbstractAlbumController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('musoundmodule_album_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'musoundmodule_album_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        $sortableColumns->addColumns([
            ...
            new Column('author'),
            new Column('uploadCover'),
            new Column('publishedDate'),
            new Column('publishedText'),
        ...
        $featureActivationHelper = $this->get('mu_sound_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            $templateParameters['items'] = $this->get('mu_sound_module.category_helper')->filterEntitiesByPermission($templateParameters['items']);
        }
     ...
     * @ParamConverter("album", class="MUSoundModule:AlbumEntity", options = {"repository_method" = "selectById", "mapping": {"id": "id"}, "map_method_signature" = true})
    ...
    protected function displayInternal(Request $request, AlbumEntity $album, $isAdmin = false)
    {
        ...
        $featureActivationHelper = $this->get('mu_sound_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
            if (!$this->get('mu_sound_module.category_helper')->hasPermission($album)) {
                throw new AccessDeniedException();
            }
        }
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_sound_module.form.handler.album');
    ...
    protected function deleteInternal(Request $request, AlbumEntity $album, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUSoundModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'album', 'id' => $album->getKey()];
        ...
        $redirectRoute = 'musoundmodule_album_' . ($isAdmin ? 'admin' : '') . 'view';
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUSoundModule', 'user' => $userName, 'action' => $action, 'entity' => 'album', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUSoundModule', 'user' => $userName, 'entity' => 'album', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUSoundModule', 'user' => $userName, 'action' => $action, 'entity' => 'album', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('musoundmodule_album_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('musoundmodule_album_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
