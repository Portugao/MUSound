...
abstract class AbstractCollectionController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('musoundmodule_collection_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'musoundmodule_collection_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
     ...
     * @ParamConverter("collection", class="MUSoundModule:CollectionEntity", options = {"repository_method" = "selectById", "mapping": {"id": "id"}, "map_method_signature" = true})
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_sound_module.form.handler.collection');
    ...
    protected function deleteInternal(Request $request, CollectionEntity $collection, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUSoundModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'collection', 'id' => $collection->getKey()];
        ...
        $redirectRoute = 'musoundmodule_collection_' . ($isAdmin ? 'admin' : '') . 'view';
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUSoundModule', 'user' => $userName, 'action' => $action, 'entity' => 'collection', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUSoundModule', 'user' => $userName, 'entity' => 'collection', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUSoundModule', 'user' => $userName, 'action' => $action, 'entity' => 'collection', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('musoundmodule_collection_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('musoundmodule_collection_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
