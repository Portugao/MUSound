...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction($objectType, $id, $source, $displayMode)
    {
        ...
        $component = 'MUSoundModule:' . ucfirst($objectType) . ':';
        if (!$this->hasPermission($component, $id . '::', ACCESS_READ)) {
            return '';
        }
        
        $entityFactory = $this->get('mu_sound_module.entity_factory');
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('mu_sound_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@MUSoundModule/External/' . ucfirst($objectType) . '/display.html.twig', $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@MUSoundModule:css/style.css'));
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('musoundmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->hasPermission('MUSoundModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('MU\SoundModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ($searchTerm != '') {
            $qb = $this->get('mu_sound_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        if (in_array($objectType, ['album'])) {
            ...
            if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
                $entities = $this->get('mu_sound_module.category_helper')->filterEntitiesByPermission($entities);
            }
        }
        ...
        $output = $this->renderView('@MUSoundModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
