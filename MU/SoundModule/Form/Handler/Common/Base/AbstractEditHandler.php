...
namespace MU\SoundModule\Form\Handler\Common\Base;
...
use MU\SoundModule\Helper\HookHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        // default to referer
        $refererSessionVar = 'musoundmodule' . $this->objectTypeCapital . 'Referer';
        ...
        $this->permissionComponent = 'MUSoundModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'MUSoundModule' . $this->objectTypeCapital . $entity->getKey();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'MUSoundModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $routeUrl = new RouteUrl('musoundmodule_' . $this->objectTypeLower . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'MUSoundModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('MUSoundModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('MUSoundModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('MUSoundModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
