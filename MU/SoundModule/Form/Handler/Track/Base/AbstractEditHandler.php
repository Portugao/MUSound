...
namespace MU\SoundModule\Form\Handler\Track\Base;

use MU\SoundModule\Form\Handler\Common\EditHandler;
use MU\SoundModule\Form\Type\TrackType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->objectTypeCapital = 'Track';
        $this->objectTypeLower = 'track';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the track yet as other items are required which must be created before!'));
                $logArgs = ['app' => 'MUSoundModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(TrackType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of track area
        $codes[] = 'userIndex';
        // admin index page of track area
        $codes[] = 'adminIndex';
        // user list of tracks
        $codes[] = 'userView';
        // admin list of tracks
        $codes[] = 'adminView';
        // user list of own tracks
        $codes[] = 'userOwnView';
        // admin list of own tracks
        $codes[] = 'adminOwnView';
        // user detail page of treated track
        $codes[] = 'userDisplay';
        // admin detail page of treated track
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $routePrefix = 'musoundmodule_' . $this->objectTypeLower . '_' . $routeArea;
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Track created.');
                } else {
                    $message = $this->__('Done! Track updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Track deleted.');
                break;
            default:
                $message = $this->__('Done! Track updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUSoundModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'track', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('musoundmodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('musoundmodule' . $this->objectTypeCapital . 'Referer');
        }
        ...
        switch ($this->returnTo) {
            ...
            case 'adminViewAlbums':
                return $this->router->generate('musoundmodule_album_' . $routeArea . 'view');
            case 'userOwnViewAlbums':
            case 'adminOwnViewAlbums':
                return $this->router->generate('musoundmodule_album_' . $routeArea . 'view', ['own' => 1]);
            case 'userDisplayAlbum':
            case 'adminDisplayAlbum':
                if (!empty($this->relationPresets['album'])) {
                    return $this->router->generate('musoundmodule_album_' . $routeArea . 'display',  ['id' => $this->relationPresets['album']]);
                }
