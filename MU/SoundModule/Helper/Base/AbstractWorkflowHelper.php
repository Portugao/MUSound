...
abstract class AbstractWorkflowHelper
{
    ...
    public function executeAction(EntityAccess $entity, $actionId = '', $recursive = false)
    {
        ...
        $logArgs = ['app' => 'MUSoundModule', 'user' => $this->currentUserApi->get('uname')];
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        $state = 'waiting';
        $objectType = 'album';
        if ($this->permissionApi->hasPermission('MUSoundModule:' . ucfirst($objectType) . ':', '::', ACCESS_ADD)) {
            $amount = $this->getAmountOfModerationItems($objectType, $state);
            if ($amount > 0) {
                $amounts[] = [
                    'aggregateType' => 'albumsApproval',
                    'description' => $this->translator->__('Albums pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->translator->_fn('One album is waiting for approval.', '%amount% albums are waiting for approval.', $amount, ['%amount%' => $amount])
                ];
        
                $this->logger->info('{app}: There are {amount} {entities} waiting for approval.', ['app' => 'MUSoundModule', 'amount' => $amount, 'entities' => 'albums']);
            }
        }
