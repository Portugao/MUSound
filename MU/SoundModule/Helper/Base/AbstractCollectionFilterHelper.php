...
use MU\SoundModule\Helper\CategoryHelper;
...
abstract class AbstractCollectionFilterHelper
{
    ...
    public function getViewQuickNavParameters($objectType = '', $context = '', $args = [])
    {
        ...
        if ($objectType == 'track') {
            return $this->getViewQuickNavParametersForTrack($context, $args);
        }
    ...
    public function addCommonViewFilters($objectType, QueryBuilder $qb)
    {
        ...
        if ($objectType == 'track') {
            return $this->addCommonViewFiltersForTrack($qb);
        }
        if ($objectType == 'collection') {
    ...
    public function applyDefaultFilters($objectType, QueryBuilder $qb, $parameters = [])
    {
        ...
        if ($objectType == 'track') {
            return $this->applyDefaultFiltersForTrack($qb, $parameters);
        }
        if ($objectType == 'collection') {
     ...
     * @return array List of template variables to be assigned
     */
    protected function getViewQuickNavParametersForTrack($context = '', $args = [])
    {
        $parameters = [];
        if (null === $this->request) {
            return $parameters;
     ...
     * @return QueryBuilder Enriched query builder instance
     */
    protected function addCommonViewFiltersForTrack(QueryBuilder $qb)
    {
        if (null === $this->request) {
            return $qb;
        }
        ...
        $parameters = $this->getViewQuickNavParametersForTrack();
        foreach ($parameters as $k => $v) {
            if (in_array($k, ['q', 'searchterm'])) {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter('track', $qb, $v);
                }
            } else if (!is_array($v)) {
                // field filter
                if ((!is_numeric($v) && $v != '') || (is_numeric($v) && $v > 0)) {
        ...
        $qb = $this->applyDefaultFiltersForTrack($qb, $parameters);
    ...
    protected function applyDefaultFiltersForAlbum(QueryBuilder $qb, $parameters = [])
    {
        ...
        $routeName = $this->request->get('_route');
        $isAdminArea = false !== strpos($routeName, 'musoundmodule_album_admin');
        if ($isAdminArea) {
            return $qb;
        }
     ...
     * @return QueryBuilder Enriched query builder instance
     */
    protected function applyDefaultFiltersForTrack(QueryBuilder $qb, $parameters = [])
    {
        if (null === $this->request) {
            return $qb;
        }
        $routeName = $this->request->get('_route');
        $isAdminArea = false !== strpos($routeName, 'musoundmodule_track_admin');
        if ($isAdminArea) {
            return $qb;
        }
    ...
    protected function applyDefaultFiltersForCollection(QueryBuilder $qb, $parameters = [])
    {
        ...
        $routeName = $this->request->get('_route');
        $isAdminArea = false !== strpos($routeName, 'musoundmodule_collection_admin');
        if ($isAdminArea) {
            return $qb;
        }
    ...
    public function addSearchFilter($objectType, QueryBuilder $qb, $fragment = '')
    {
        ...
        if ($objectType == 'album') {
            ...
            $filters[] = 'tbl.uploadCover = :searchUploadCover';
            $parameters['searchUploadCover'] = $fragment;
            $filters[] = 'tbl.publishedDate = :searchPublishedDate';
            $parameters['searchPublishedDate'] = $fragment;
            $filters[] = 'tbl.publishedText LIKE :searchPublishedText';
            $parameters['searchPublishedText'] = '%' . $fragment . '%';
        }
        if ($objectType == 'track') {
            $filters[] = 'tbl.title LIKE :searchTitle';
            $parameters['searchTitle'] = '%' . $fragment . '%';
            ...
            $filters[] = 'tbl.author LIKE :searchAuthor';
            $parameters['searchAuthor'] = '%' . $fragment . '%';
            $filters[] = 'tbl.uploadTrack = :searchUploadTrack';
            $parameters['searchUploadTrack'] = $fragment;
            $filters[] = 'tbl.uploadZip = :searchUploadZip';
            $parameters['searchUploadZip'] = $fragment;
        }
        if ($objectType == 'collection') {
            $filters[] = 'tbl.title LIKE :searchTitle';
            $parameters['searchTitle'] = '%' . $fragment . '%';
