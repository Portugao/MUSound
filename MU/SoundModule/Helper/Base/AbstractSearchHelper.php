...
abstract class AbstractSearchHelper implements SearchableInterface
{
    ...
    {
        if (!$this->permissionApi->hasPermission('MUSoundModule::', '::', ACCESS_READ)) {
            return '';
        }
        ...
        foreach ($searchTypes as $searchTypeCode => $typeInfo) {
            ...
            switch ($objectType) {
                case 'album':
                    ...
                    $whereArray[] = 'tbl.uploadCover';
                    $whereArray[] = 'tbl.publishedText';
                    break;
                case 'track':
                    $whereArray[] = 'tbl.workflowState';
                    $whereArray[] = 'tbl.title';
                    $whereArray[] = 'tbl.description';
                    $whereArray[] = 'tbl.author';
                    $whereArray[] = 'tbl.uploadTrack';
                    $whereArray[] = 'tbl.uploadZip';
                    break;
                case 'collection':
                    $whereArray[] = 'tbl.workflowState';
                    $whereArray[] = 'tbl.title';
            ...
            $entitiesWithDisplayAction = ['album', 'track', 'collection'];
            ...
            foreach ($entities as $entity) {
                ...
                if (!$this->permissionApi->hasPermission('MUSoundModule:' . ucfirst($objectType) . ':', $entity->getKey() . '::', ACCESS_OVERVIEW)) {
                    continue;
                }
                ...
                $displayUrl = $hasDisplayAction ? new RouteUrl('musoundmodule_' . strtolower($objectType) . '_display', $urlArgs) : '';
    
                $result = new SearchResultEntity();
                $result->setTitle($formattedTitle)
                    ->setText($description)
                    ->setModule('MUSoundModule')
                    ->setCreated($created)
    ...
    protected function getSearchTypes()
    {
        $searchTypes = [
            ...
            'mUSoundModuleTracks' => [
                'value' => 'track',
                'label' => $this->__('Tracks')
            ],
            'mUSoundModuleCollections' => [
                'value' => 'collection',
