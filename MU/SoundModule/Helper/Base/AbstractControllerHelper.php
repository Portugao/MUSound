...
use MU\SoundModule\Helper\ImageHelper;
use MU\SoundModule\Helper\ModelHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in MUSoundModule.
     *
    ...
    /**
     * Returns the default object type in MUSoundModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('MUSoundModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('MUSoundModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('MU\SoundModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('musoundmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('musoundmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'album') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'UploadCover'] = $this->imageHelper->getRuntimeOptions($objectType, 'uploadCover', $context, $args);
                $parameters['thumbRuntimeOptions'] = $thumbRuntimeOptions;
            }
            if ($objectType == 'track') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'UploadTrack'] = $this->imageHelper->getRuntimeOptions($objectType, 'uploadTrack', $context, $args);
                $thumbRuntimeOptions[$objectType . 'UploadZip'] = $this->imageHelper->getRuntimeOptions($objectType, 'uploadZip', $context, $args);
                $parameters['thumbRuntimeOptions'] = $thumbRuntimeOptions;
            }
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                // use separate preset for images in related items
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'MUSoundModule_relateditem', $context, $args);
            }
        }
