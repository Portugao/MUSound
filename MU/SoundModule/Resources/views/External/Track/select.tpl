...
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Track'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='track' from=$items}
                        <option value="{$track->getKey()}"{if $selectedId eq $track->getKey()} selected="selected"{/if}>{$track->getTitle()}</option>
                    {foreachelse}
