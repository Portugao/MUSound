...
function mUSoundTriggerFormValidation()
{
    mUSoundExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function mUSoundHandleFormSubmit (event) {
    if (triggerValidation) {
        mUSoundTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
            event.preventDefault();
            return false;
        }
 ...
 */
function mUSoundInitEditForm(mode, entityId)
{
    if (jQuery('.musound-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.musound-edit-form').first();
    ...
    editForm.submit(mUSoundHandleFormSubmit);
