 ...
 */
function mUSoundExecuteCustomValidationConstraints(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!mUSoundValidateNoSpace(jQuery(this).val())) {
    ...
    jQuery('.validate-upload').each( function() {
        if (!mUSoundValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
